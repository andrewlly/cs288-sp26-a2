...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: Tensor, token_positions: Optional[Tensor] = None) -> Tensor:
        ...
        """
        norm1 = self.ln1(x)
        attn_out = self.attn(norm1, token_positions)
        x = x + attn_out
...
def count_flops_per_token(
    ...
    qkv_out_proj_macs = 4 * (d_model ** 2)
    attn_macs = 2 * context_length * d_model
    

    ffn_macs = 3 * d_model * d_ff
    layer_macs = qkv_out_proj_macs + attn_macs + ffn_macs
    
    head_macs = d_model * vocab_size
    total_macs = (num_layers * layer_macs) + head_macs
    return 2 * total_macs
...
def estimate_memory_bytes(
    ...
    layer_params = norm_params + attn_params + ffn_params
    total_layer_params = num_layers * layer_params
    
    final_norm_params = d_model
    
    output_params = d_model * vocab_size
    total_params = embedding_params + total_layer_params + final_norm_params + output_params
